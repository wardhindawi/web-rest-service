...
public class GenricExceptionMapper implements ExceptionMapper<Exception> {
    ...
    public Response toResponse(Exception e) {
        ...
        System.out.println("Generic expection mapper " + e);
        ...
        if (e.getMessage() != null && e.getMessage().contains(HTTP_UNSUPPORTED_MEDIA_TYPE)) {
            ...
            response = Response.status(Status.UNSUPPORTED_MEDIA_TYPE).entity(error).type(MediaType.APPLICATION_JSON)
                    .build();
            ...
            response = Response.status(Status.METHOD_NOT_ALLOWED).entity(error).type(MediaType.APPLICATION_JSON)
                    .build();
            ...
            response = Response.status(Status.INTERNAL_SERVER_ERROR).entity(error).type(MediaType.APPLICATION_JSON)
                    .build();
        }
